package nexus.io.tio.boot.admin.consts;

public interface AppConstant {

  String APP_ADMIN_SECRET_KEY = "app.admin.secret.key";
  String APP_ADMIN_TOKEN = "app.admin.token";
  String APP_TOKEN_TIMEOUT = "app.token.timeout";
  String APP_STORAGE_PLATFORM = "app.storage.platform";
  String APP_AREA_CODE = "app_area_code";
  String APP_WARM_NOTIFICATION_WEBHOOK_URL = "app.warm.notification.webhook.url";

}
